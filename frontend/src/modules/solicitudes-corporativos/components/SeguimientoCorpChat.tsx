...
import { addSeguimientoCorp, listSeguimientosCorp } from '../services/seguimientoCorpService'
...
type TimelineItem =
  ...
  | { type: 'msg'; key: string; entry: SeguimientoCorpEntry }

function buildTimeline(entries: SeguimientoCorpEntry[]): TimelineItem[] {
...
export default function SeguimientoCorpChat({
  solicitudId,
  currentUserId,
  autorNombre,
  pendingMessage = '',
  ...
  const [entries, setEntries] = useState<SeguimientoCorpEntry[]>([])
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    setLoadFailed(false)
    const { data, error: err } = await listSeguimientosCorp(solicitudId)
    if (err) {
      setError(err)
      setLoadFailed(true)
      setEntries([])
  ...
  const handleSend = async () => {
    ...
    setSending(true)
    setError(null)
    const { data, error: err } = await addSeguimientoCorp(solicitudId!, currentUserId, autorNombre, text)
    setSending(false)
    if (err) {
      setError(err)
      return
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-50/50 dark:bg-slate-800/30">
      <div className="px-4 py-4 space-y-3" aria-live="polite" aria-label="Historial de seguimientos">
        {loading && (
        ...
        {!loading && isNew && entries.length === 0 && (
          <p className="text-center text-xs text-slate-500 dark:text-slate-400 py-6 px-2">
            Al registrar la solicitud podrá guardar el primer seguimiento. Los siguientes se agregan aquí con fecha y autor.
          </p>
        )}
          ...
          if (item.type === 'day') {
            return (
              <div key={item.key} className="flex justify-center py-1">
                <span
                  className="text-[10px] font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400
      ...
      {!readOnly && (
        <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/80 p-3">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 text-right">
            ...
            {isNew && draft.trim() ? ' · Se guardará al registrar la solicitud' : ''}
          </p>
        </div>
      )}
    </div>
