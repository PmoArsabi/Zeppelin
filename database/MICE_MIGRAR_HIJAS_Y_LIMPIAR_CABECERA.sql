-- =============================================================================
-- ZEPPELIN MICE — Migrar texto legacy → tablas hijas + limpiar cabecera
-- (SCRIPT CANÓNICO — mantener este archivo al editar SQL)
--
-- Alias equivalente: MICE_EJECUTAR_TODO.sql (mismo contenido, punto de entrada en docs).
--
-- Ejecutar UNA vez en Supabase / DBeaver (SQL Editor → Run).
-- Idempotente: no duplica hijas si ya existen; DROP COLUMN IF EXISTS.
--
...
-- FKs a id bloquean cambiar PK del catálogo; se recrean en paso C
alter table public.th_solicitud_mice
...
-- =============================================================================
-- H) Vista resumen (debe recrearse ANTES de DROP columnas legacy)
-- =============================================================================
...
-- Solicitudes que aún no tienen hijas (revisar manualmente si aparecen filas)
select
  s.mzp,
  s.cliente_id,
  s.estado_id,
